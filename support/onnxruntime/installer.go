...
const (
	...
	ROCmWheelRepoURL = "https://repo.radeon.com/rocm/manylinux"
)
...
func InstallMigraphx(rocmVersion string, targetDir string, force bool) (string, error) {
	...
	if installDir == "" {
		var err error
		installDir, err = GetMigraphxInstallPath()
		if err != nil {
			return "", err
		}
	} else {
	...
	targetPath := filepath.Join(installDir, libFilename)
	migraphxLibPath := filepath.Join(installDir, "libonnxruntime_providers_migraphx.so")
	if !force {
		if _, err := os.Stat(targetPath); err == nil {
	...
	for _, file := range zr.File {
		...
		targetFile := filepath.Join(installDir, baseName)
		f, err := os.OpenFile(targetFile, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0755)
		if err != nil {
			return "", errors.Wrapf(err, "failed to create file %s", targetFile)
		}
